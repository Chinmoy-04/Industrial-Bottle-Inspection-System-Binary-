...
def train_fold(model_name, fold, train_df, val_df, base_dir, img_name_to_bbox):
    run_dir = os.path.join(base_dir, "runs", "classify", "krones_challenge", f"{model_name}_v7_fold_{fold}")
    ...
    for epoch in range(start_epoch, epochs):
        ...
        print(f"Epoch {epoch+1}/{epochs} - Train Loss: {train_loss:.4f} | Val Loss: {val_loss:.4f} | Val F1: {epoch_best_f1:.4f}")
...
def main():
    print("Initializing PyTorch timm Pipeline (V7 - Pseudo-Labeling + RandAugment)...")
